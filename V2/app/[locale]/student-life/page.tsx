...
export default async function StudentLifePage({
  ...
  const t = await getTranslations('StudentLifePage');
    ...
    const activities = [
      { key: 'arts', icon: Palette, color: '#6D28D9' },
      { key: 'music', icon: Music, color: '#C41E3A' },
      { key: 'sports', icon: Trophy, color: '#B45309' },
      { key: 'leadership', icon: Award, color: '#0F766E' },
    ];
        ...
        <section className="py-24 bg-white">
          <div className="container-custom">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                            {activities.map(({ key, icon: Icon, color }) => (
